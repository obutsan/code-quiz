    ...
    return (
      <>
        <div className="game-wrapper vw-80 d-flex align-items-center flex-column" >
            ...
            <h2 className="welcome-text bangers-text h2 text-uppercase mb-0 ms-4 d-none d-md-block">Welcome to HTML Forge 💥</h2>
           ...
           <img className="img-fluid object-fit-contain responsive-img" src="/assets/HtmlGame/htmlstart.jpeg" alt="coding man" />
          </div>
              ...
              <div className="d-flex flex-column mt-4">
                ...
                <Button style={{height: '3em'}} onClick={() => onAnswerSelected(answer, index)} key={answer} id={selectedAnswerIndex === index ? 'selected-answer' : null } className="m-1 answer-button border border-5"><p className="fw-bold  h5 button-text">{answer}</p></Button>
                ))}
              </div>  
                  ...
                  <div className="mt-2 h2 bangers-text">
								    {alertMessage}
                  </div>
                </div>    
							  <div>
                 { show ? <button  className="shaking-button bangers-text m-2 ms-5 bg-warning"onClick={navigateToLevel3}>GO TO NEXT LEVEL</button> : null}
                 { showRestart ? <button  className="shaking-button bangers-text m-2 ms-5 bg-warning"onClick={handleRestart}>TRY AGAIN!</button> : null}
						    </div>
						  </div>      
            </div>
          </div>
